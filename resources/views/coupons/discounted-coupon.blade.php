        ...
        <div class="row">
            <div class="coll-2">
                <form action="{{ route('search-coupon') }}" method="GET" class="search__form mb-3">
                    <input type="text" placeholder="Search Coupon.." class="search__input" name="searchText" id="query"
                        value="" autocorrect="off" class="search-box-home-page" autocapitalize="off" autocomplete="off"
                        data-autocomplete="search">
                    <button type="submit" class="button-search"><i class="fa fa-search"></i></button>
                    @error('searchText')
                        <div class="dispri" id="serError">
                            {{ $message }}
                        </div>
                    @enderror
                </form>
                <div class="check__boxes">
                    <a href="{{ route('discounted') }}" class="check__box">
                        <input type="checkbox" name="" id=""class="check_box1">
            ...
            <div id="wrapper">
                <div class="main__content">
                    <h1 class="mb-5">Discounted Coupons</h1>
                    <div id="hide_content">
                        @isset($couponsData)
                            @foreach ($couponsData as $coupon)
                                <div class="d-flex mb-4">
                                    ...
                                    <span class="me-2 nearby">( <del
                                            class="actpr">${{ $coupon->actual_price }}</del>, <span
                                            class="dispri">${{ $coupon->discounte_price }}</span> )</span>
                                </div>
                            @endforeach
                        @endisset
                    </div>
